...
exports.run = async(client, message, args, ayar, emoji) => {
  ...
  message.channel.bulkDelete(Number(args[0])).then(msjlar => message.channel.send(embed.setDescription(`${client.config.murphyTag} Başarıyla **${msjlar.size}** adet mesaj silindi!`)).then(x => x.delete({timeout: 5000}))).catch()
};
exports.conf = {
    aliases: ["sil", 'clear'],
    usage: "temizle 1-100",
