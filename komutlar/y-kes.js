...
message.guild.member(member.id).voice.setChannel(null)
   ...
   message.channel.send(new MessageEmbed().setDescription(`${client.config.murphyTag} ${member} kullancısının ${message.author} tarafından bağlantısı kesildi.`).setFooter("Deasn was here!").setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true })).setColor('#7289D').setTimestamp()).then(x => x.delete({timeout: 5000}));
}
exports.conf = { 
enabled: true, 
guildOnly: true, 
